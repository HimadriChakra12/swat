...
void rendering_init(void) {
    int had_driver = getenv("SDL_VIDEODRIVER") != NULL;
    if (!had_driver) setenv("SDL_VIDEODRIVER", "wayland", 0);
    ...
    wnd = SDL_CreateWindow(RENDERING_DEFAULT_WINDOW_TITLE, wnd_rect.x, wnd_rect.y, wnd_rect.w, wnd_rect.h, wnd_flags);
    if (wnd == NULL) {
