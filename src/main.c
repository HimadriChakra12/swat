...
static void build_save_path(const char* dir, char* out, size_t outsz) {
    ...
    char base[256];
    strftime(base, sizeof(base), "swat_%Y%m%d_%H%M%S", tm);
...
static void do_copy_image(void) {
    /* write to a temp file then pipe to wl-copy (wl-clipboard) */
    char tmp[] = "/tmp/swat_copy_XXXXXX.png";
    /* mkstemp doesn't support suffix so just use a fixed tmp path */
    snprintf(tmp, sizeof(tmp), "/tmp/swat_copy_%d.png", (int)getpid());
    ...
    snprintf(cmd, sizeof(cmd),
        "wl-copy -t image/png < \"%s\" && rm -f \"%s\"", tmp, tmp);
    if (system(cmd) == 0)
        fprintf(stderr, "image copied to clipboard\n");
    else
        fprintf(stderr, "copy failed — is wl-clipboard installed?\n");
}
...
int main(int argc, char* argv[]) {
    ...
    /* argument parsing:
     *   swat <image>          plain open
     *   swat -f <image>       open fullscreen  (swappy-compat / dihshot default)
     *   swat -r <image>       open fullscreen + rect tool
     *   swat -p <image>       open fullscreen + pencil tool
     *   swat -m <image>       open fullscreen + marker tool
     *   cat img | swat        stdin pipe
     */
    ...
    if (path == NULL && !stdin_has_data()) {
        ...
        int ready = !isatty(STDIN_FILENO)
                    ...
                    : 0;
        if (ready <= 0) {
            fprintf(stderr, "usage: swat [-f|-r|-p|-m] <image>\n"
                            "       -f  fullscreen\n"
